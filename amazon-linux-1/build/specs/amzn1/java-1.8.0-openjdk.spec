...
# We disable debug build on amzn
#%ifarch %{jit_arches}
...
%global sdklnk_noarch() %{expand:java-%{javaver}-%{origin}%1}
%global jrelnk_noarch() %{expand:jre-%{javaver}-%{origin}%1}

%global sdkdir()        %{expand:%{uniquesuffix %%1}}
%global sdklnk()        %{expand:%{sdklnk_noarch %%1}.%{_arch}%1}
%global jrelnk()        %{expand:%{jrelnk_noarch %%1}.%{_arch}%1}

%global jredir()        %{expand:%{sdkdir %%1}/jre}
%global sdkbindir()     %{expand:%{_jvmdir}/%{sdklnk %%1}/bin}
%global jrebindir()     %{expand:%{_jvmdir}/%{jrelnk %%1}/bin}
...
if [ "$1" -gt 1 ]; then
  ...
  if [ "${sum}" = '1690ac33955594f71dc952c9e83fd396' -o \\
       ...
       "${sum}" = 'a59c6d96aeae1303fb8ba85e97588e9d' -o \\
       "${sum}" = '3cd782982a47aa06399f5258ede65577' -o \\
       "${sum}" = 'f87e2b039e6d5a6b34c451e4f9f18dfb' ]; then
...
alternatives \\
  ...
  --slave %{_jvmdir}/jre jre %{_jvmdir}/%{jrelnk %%1} \\
...
for X in %{origin} %{javaver} ; do
  alternatives \\
    ...
    jre_"$X" %{_jvmdir}/%{jrelnk %%1} $PRIORITY  \\
    --slave %{_jvmjardir}/jre-"$X" \\
    jre_"$X"_exports %{_jvmjardir}/%{jrelnk %%1}
done
...
if [ $1 -eq 0 ]; then
  ...
  alternatives --remove jre_%{origin} %{_jvmdir}/%{jrelnk %%1}
  alternatives --remove jre_%{javaver} %{_jvmdir}/%{jrelnk %%1}
fi
}
...
alternatives \\
  ...
  --slave %{_jvmdir}/java java_sdk %{_jvmdir}/%{sdklnk %%1} \\
  --slave %{_jvmjardir}/java java_sdk_exports %{_jvmjardir}/%{sdklnk %%1} \\
...
for X in %{origin} %{javaver} ; do
  alternatives \\
    ...
    java_sdk_"$X" %{_jvmdir}/%{sdklnk %%1} $PRIORITY  \\
    --slave %{_jvmjardir}/java-"$X" \\
    java_sdk_"$X"_exports %{_jvmjardir}/%{sdklnk %%1}
done
...
if [ $1 -eq 0 ] ; then
  ...
  alternatives --remove java_sdk_%{origin} %{_jvmdir}/%{sdklnk %%1}
  alternatives --remove java_sdk_%{javaver} %{_jvmdir}/%{sdklnk %%1}
fi
...
%{_jvmdir}/%{sdklnk %%1}
%{_jvmdir}/%{sdklnk_noarch %%1}
%{_jvmjardir}/%{sdkdir %%1}
%{_jvmjardir}/%{sdklnk %%1}
%{_jvmjardir}/%{sdklnk_noarch %%1}
%if %{with X11}
...
# Java requires some fonts to be installed on the system for the font API to work
# People in headless environments usally don't have any fonts installed so this ensures
# everything will continue to work seamlessly for the user
# See https://forums.aws.amazon.com/thread.jspa?messageID=308415
...
# Java 8 support was added in 1.7.5-27
Requires: jpackage-utils >= 1.7.5-27
...
Requires: lksctp-tools%{?_isa}, cups-libs%{?_isa}
...
#Amzn1 - to build against our version of xorg
...
%endif
  ...
  pushd $RPM_BUILD_ROOT%{_jvmdir}
    ...
    ln -sf %{sdkdir $suffix} %{sdklnk $suffix}
    ln -sf %{jrelnk $suffix} %{jrelnk_noarch $suffix}
    ln -sf %{sdklnk $suffix} %{sdklnk_noarch $suffix}
  popd
...
# Create symlinks so java-1.8.0-openjdk can still be easily used while another jvm
# is the default system wide
%__mkdir_p %{buildroot}%{_bindir}
for command in $(find %{buildroot}%{jrebindir %{nil}} %{buildroot}%{sdkbindir %{nil}} \
 -perm +003 -type f); do
    command_name=$(basename ${command})
    path_name="%{buildroot}%{_bindir}/${command_name}8"
    real_path=${command#$RPM_BUILD_ROOT}
    if [ ! -h ${path_name} ]; then
        %__ln_s ${real_path} ${path_name}
        echo "%{_bindir}/${command_name}8" >> %{name}.files-headless
    fi
done
...
%triggerpostun headless -- java-1.8.0-openjdk-headless  <= 1:1.8.0.31-2.b13.7.amzn1
# Previous versions of this package used (and removed) an alternatives symlink
# at %{_jvmdir}/jre-%{javaver}-%{origin}. This is changed to a symlink to
# %{_jvmdir}/%{jrelnk %{nil}} after this release, so we need to ensure the new symlink
# exists after the old alternative-based symlink is removed.
ln -sf %{_jvmdir}/%{jrelnk %{nil}} %{_jvmdir}/%{jrelnk_noarch %{nil}}
...
%triggerpostun devel -- java-1.8.0-openjdk-devel  <= 1:1.8.0.31-2.b13.7.amzn1
# Previous versions of this package used (and removed) an alternatives symlink
# at %{_jvmdir}/java-%{javaver}-%{origin}. This is changed to a symlink to
# %{_jvmdir}/%{sdklnk %{nil}} after this release, so we need to ensure the new symlink
# exists after the old alternative-based symlink is removed.
ln -sf %{_jvmdir}/%{sdklnk %{nil}} %{_jvmdir}/%{sdklnk_noarch %{nil}}
