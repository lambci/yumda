...
%configure --enable-dynamic --with-fontpath="$FONTPATH" \
        --with-drivers=FILES --disable-compile-inits \
        --disable-cups --disable-gtk \
        --without-ijs --without-pdftoraster --without-x \
...
rm -rf $RPM_BUILD_ROOT%{_datadir}/%{name}/%{gs_dot_ver}/Resource/Font

rm -rf $RPM_BUILD_ROOT/%{_bindir}/gsx
mkdir -p $RPM_BUILD_ROOT%{_datadir}/%{name}/conf.d
mkdir -p $RPM_BUILD_ROOT%{_sysconfdir}/%{name}/%{gs_dot_ver}
touch $RPM_BUILD_ROOT%{_sysconfdir}/%{name}/%{gs_dot_ver}/Fontmap.local
touch $RPM_BUILD_ROOT%{_sysconfdir}/%{name}/%{gs_dot_ver}/cidfmap.local
touch $RPM_BUILD_ROOT%{_sysconfdir}/%{name}/%{gs_dot_ver}/CIDFnmap.local

# Don't ship fixmswrd.pl as it pulls in perl (bug #463948).
rm -f $RPM_BUILD_ROOT%{_bindir}/fixmswrd.pl

MAIN_PWD=`pwd`
(cd $RPM_BUILD_ROOT; find .%{_datadir}/%{name}/%{gs_dot_ver}/Resource -type f | \
...
%dir %{_sysconfdir}/%{name}/%{gs_dot_ver}
%dir %{_datadir}/%{name}
%dir %{_datadir}/%{name}/conf.d
%dir %{_datadir}/%{name}/%{gs_dot_ver}
%dir %{_datadir}/%{name}/%{gs_dot_ver}/Resource
%dir %{_datadir}/%{name}/%{gs_dot_ver}/Resource/Init
%config %{_datadir}/%{name}/%{gs_dot_ver}/Resource/Init/gs_init.ps
%config %{_datadir}/%{name}/%{gs_dot_ver}/Resource/Init/Fontmap*
%{_datadir}/%{name}/%{gs_dot_ver}/lib
%{_libdir}/libgs.so.*
%dir %{_libdir}/%{name}
%{_libdir}/%{name}/%{gs_dot_ver}
%config(noreplace) %{_sysconfdir}/%{name}/%{gs_dot_ver}/*

%exclude %{_mandir}
%exclude %{_includedir}
%exclude %{_defaultdocdir}
%exclude %{_datadir}/%{name}/%{gs_dot_ver}/examples
%exclude %{_libdir}/*.so
