...
%configure \
    ...
    --with-logfile="%{_localstatedir}/log/snmpd.log" \
...
%endif
    ...
    --enable-as-needed \
    --with-perl-modules="INSTALLDIRS=vendor PREFIX=%{_prefix} INSTALLVENDORLIB=%{perl_vendorlib} INSTALLVENDORARCH=%{perl_vendorarch}" \
    --enable-mfd-rewrites \
    --enable-local-smux \
...
ln -s snmptrap ${RPM_BUILD_ROOT}%{_bindir}/snmpinform
...
%exclude %{_initrddir}/snmpd
%exclude %{_initrddir}/snmptrapd
...
%exclude %{_bindir}/net-snmp-cert
%exclude %{_datadir}/snmp/mib2c*
%exclude %{_datadir}/snmp/*.pl
%exclude %{_bindir}/traptoemail
%exclude %{_bindir}/tkmib
