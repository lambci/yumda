...
ln -s /usr/include/nss3 include/nss
ln -s /usr/include/nspr4 include/nspr

AUTOMAKE=true autoreconf -fi
...
pushd %{buildroot}
rm -rf .%{_includedir} .%{_mandir} .%{_libdir}/{*.la,*.so,pkgconfig}
popd


%post
%{_libexecdir}/openldap/create-certdb.sh -d %{_sysconfdir}/openldap/certs >&/dev/null || :

%post servers
# generate sample TLS certificate for server (will not replace)
%{_libexecdir}/openldap/generate-server-cert.sh -d %{_sysconfdir}/openldap/certs -o &>/dev/null || :
