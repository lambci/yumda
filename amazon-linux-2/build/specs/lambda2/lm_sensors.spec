...
make PREFIX=%{_prefix} LIBDIR=%{_libdir} MANDIR=%{_mandir} ETCDIR=%{_sysconfdir} BUILD_STATIC_LIB=0 \
     EXLDFLAGS="-pie -Wl,-z,relro,-z,now" PROG_EXTRA=sensord user


%install
make PREFIX=%{_prefix} LIBDIR=%{_libdir} MANDIR=%{_mandir} ETCDIR=%{_sysconfdir} BUILD_STATIC_LIB=0 PROG_EXTRA=sensord \
...
sed -i '1 s|^#!/usr/bin/perl|#!%{_bindir}/perl|' %{buildroot}{%{_bindir},%{_sbindir}}/*
