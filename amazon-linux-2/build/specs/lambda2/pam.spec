...
make install DESTDIR=$RPM_BUILD_ROOT LDCONFIG=: LOCKDIR=%{_prefix}/var/run/console
...
install -d -m 755 $RPM_BUILD_ROOT%{_prefix}/var/log
install -m 600 /dev/null $RPM_BUILD_ROOT%{_prefix}/var/log/tallylog
install -d -m 755 $RPM_BUILD_ROOT%{_prefix}/var/run/faillock
...
%dir %{_prefix}/var/run/console
%if %{WITH_SELINUX}
%config(noreplace) %{_secconfdir}/sepermit.conf
%dir %{_prefix}/var/run/sepermit
%endif
%ghost %verify(not md5 size mtime) %{_prefix}/var/log/tallylog
%dir %{_prefix}/var/run/faillock
%{_prefix}/lib/tmpfiles.d/pam.conf

%exclude %{_mandir}
%exclude %{_includedir}
%exclude %{_libdir}/libpam.so
%exclude %{_libdir}/libpamc.so
%exclude %{_libdir}/libpam_misc.so
