...
%if %{use_filesystem_pkg}
# to ensure we are using httpd with filesystem feature: https://bugzilla.redhat.com/show_bug.cgi?id=1081453
BuildRequires: httpd-filesystem
# to ensure we are using nginx with filesystem feature: https://bugzilla.redhat.com/show_bug.cgi?id=1142298
...
%pre fpm
getent group apache >/dev/null || /usr/sbin/groupadd -g 48 -r apache
getent passwd apache >/dev/null || \
    /usr/sbin/useradd -c "Apache" -u 48 -g apache \
        -s /sbin/nologin -r -d /usr/share/httpd apache >/dev/null || :

getent group nginx > /dev/null || /usr/sbin/groupadd -r nginx
getent passwd nginx > /dev/null || \
    /usr/sbin/useradd -r -d %{_localstatedir}/lib/nginx -g nginx \
        -s /sbin/nologin -c "Nginx web server" nginx >/dev/null || :
